		...
		mPreview.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (mCamera != null) {
					...
					if (parameters.getFocusMode() != Parameters.FOCUS_MODE_AUTO) {
					...
					try {
						...
						camera.autoFocus(new Camera.AutoFocusCallback() {
							...
							public void onAutoFocus(boolean success,
									Camera camera) {
								if (camera.getParameters().getFocusMode() != Parameters.FOCUS_MODE_CONTINUOUS_PICTURE) {
...
//				MySingleton.getInstance().mBitmapByteArray = data;
